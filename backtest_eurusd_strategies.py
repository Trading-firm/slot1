...
# Settings
SYMBOLS = ["EURUSD", "EUR/USD"] # Try both formats
TIMEFRAME_NAME = "1h"
...
def get_data(symbol_list, timeframe, n=1000): 
    ...
    if rates is None or len(rates) == 0:
        print(f"Failed to get data (tried {symbol_list}): {mt5.last_error()}", flush=True)
        return None, None
...
def run_backtest(tf_name, timeframe_enum):
    df, symbol = get_data(SYMBOLS, timeframe_enum)
    if df is None:
        return None
...
if __name__ == "__main__":
    res = run_backtest(TIMEFRAME_NAME, TIMEFRAME_ENUM)
    
    if res:
        print(f"\n--- FINAL SUMMARY: EUR/USD (Sorted by PnL) ---")
        ...
        for strat_name, stats in sorted_strategies:
            ...
            print(f"  {strat_name:<25}: {total:>3} trades, {wr:>5.1f}% WR, PnL: {pnl:>8.5f}")
    else:
        print(f"No results for EUR/USD")
