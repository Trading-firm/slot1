...
def run_backtest(symbol, tf_name, timeframe_enum):
    ...
    variations = {
        ...
        "Any Single + RSI Filter": {"wins": 0, "losses": 0, "trades": [], "skip_until": 0},
    ...
    for i in range(300, len(df)):
        ...
        for var_name in variations:
            ...
            # Any Single + RSI Filter
            elif var_name == "Any Single + RSI Filter":
                # Check for ANY signal
                has_buy = buy_votes >= 1
                has_sell = sell_votes >= 1
                
                # Apply RSI + EMA 200 Safety
                # Buy: RSI < 70 AND Price > EMA 200 (Trend is UP)
                # Sell: RSI > 30 AND Price < EMA 200 (Trend is DOWN)
                ...
                # If both are true (rare), prioritize based on stronger signal or just skip?
                # For now, let's just take the first valid one or skip if both valid (conflict)
